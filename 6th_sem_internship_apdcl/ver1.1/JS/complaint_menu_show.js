...
function buttonClickHandler(event) {
    ...
    if (buttonValue === 'voltage_fluctuation') {
        ...
        lodgeComplaint("Voltage fluctuation in my electricity connection",consumer_number,getCurrentDate())
    } 
    else if (buttonValue === 'incorrect_readings') {
        complaintMenuBtnOptions.style.display = "none";
        appendChats("My meter shows Incorrect readings","user")
        lodgeComplaint("My meter shows Incorrect readings",consumer_number,getCurrentDate())
    } 
    else if (buttonValue === 'payment_error') {
        complaintMenuBtnOptions.style.display = "none";
        appendChats("There was a payment error in my last payment","user")
        lodgeComplaint("There was a payment error in my last payment",consumer_number,getCurrentDate())
        console.log("payment");
    } 
